...
# Page configuration
st.set_page_config(
    page_title="Bikes & Snow Agent Chat",
    page_icon="🏔️",
    layout="wide"
)
...
def create_thread():
    ...
    request_body = {
        "origin_application": "bikes_snow_agent"
    }
...
def call_agent_with_thread(prompt, thread_data):
    ...
    url = f"https://{host}/api/v2/databases/{database}/schemas/{schema}/agents/BIKES_SNOW_AGENT:run"
...
def main():
    # ----------------------------- Page content ------------------------------
    st.title("🏔️ Bikes & Snow Agent Chat")
    st.markdown("Powered by Snowflake Agents API!")
    ...
    # Chat interface
    st.header("💬 Ask any question about our Bikes & Snow Products specs, sales or customer comments")
